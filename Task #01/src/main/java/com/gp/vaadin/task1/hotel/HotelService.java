...
public class HotelService {
	...
	public synchronized List<Hotel> findAll(String filterName, String filterAddress) {
		...
		for (Hotel hotel : hotels.values()) {
			try {
				boolean passesFilterName = (filterName == null || filterName.isEmpty())
						|| hotel.getName().toLowerCase().contains(filterName.toLowerCase());
				boolean passesFilterAddress = (filterAddress == null || filterAddress.isEmpty())
						|| hotel.getAddress().toLowerCase().contains(filterAddress.toLowerCase());
				if (passesFilterName && passesFilterAddress) {
					arrayList.add(hotel.clone());
				}
			} catch (CloneNotSupportedException ex) {
				Logger.getLogger(HotelService.class.getName()).log(Level.SEVERE, null, ex);
	...
	public void ensureTestData() {
		if (findAll().isEmpty()) {
			final String[] hotelData = new String[] {
					...
					"Khongview Guesthouse;1;https://www.booking.com/hotel/la/khongview-guesthouse.en-gb.html;Ban Klang Khong, Khong District, 01000 Muang KhГґng, Laos",
